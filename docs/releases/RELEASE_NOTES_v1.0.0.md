...
- GPU EHSS plumbing and parity/thermalization sanity checks retained for developers, but the primary runtime GPU path is **disabled for v1.0.0** (CPU-only in `SimulationEngine`; other GPU helpers remain buildable for dev/testing).
...
## Validation (CPU v1.0.0)
- Full physics validation is available in `validation/VALIDATION_REPORT_v1.0.0.md` (thermalization, IMS, Orbitrap, LQIT, quadrupole, FT-ICR, TOF, drift/gas flow, combined drift, reactions, space charge, CPU performance).
- GPU performance suite is skipped because the runtime GPU path is disabled in v1.0.0; configs remain for future releases.
...
- **GPU status:** GPU collision (EHSS/HSS) and GPU integration are experimental; the primary runtime GPU path in `SimulationEngine` is disabled for v1.0.0. CUDA builds still compile helpers for dev/testing, with CPU fallback when batches are unavailable.
- **Magnetic field providers:** Analytical/uniform B-fields work; external B-field map providers are stubbed out and not wired in v1.0.0.
