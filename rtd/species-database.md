...
| `ipm_samples_file` | path | Self-describing `InteractionPotentialModel` HDF5 table containing numerical lookup data, resolved settings, input hashes and embedded inputs, RNG provenance, and completion state. See [Interaction-potential precomputation](ipm-precomputation.md). |
...
For publication data, archive the exact species database together with the config and output file. When `InteractionPotentialModel` is used, also archive the exact IPM HDF5 file: the species database alone does not preserve the collision table that formed part of the scientific model.
