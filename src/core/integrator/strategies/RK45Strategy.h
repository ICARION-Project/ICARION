...
public:
   ...
   */
  void step(core::IonEnsemble &ensemble, size_t ion_idx, double t, double dt,
   ...
   * - Internal state: per-ion FSAL/k1 cache (not thread-safe)
   */
  void step_adaptive(IonState &ion, double t, double &dt_inout,
...
private:
   ...
   */
  void compute_acceleration_batch(double &ax, double &ay, double &az,
                                  const core::IonEnsemble &ensemble,
                                  size_t ion_idx, double t,
                                  const physics::ForceRegistry &force_registry);

  void
  compute_acceleration_state(double &ax, double &ay, double &az,
                             const IonState &state, double t,
                             const physics::ForceRegistry &force_registry,
                             std::optional<size_t> ion_index = std::nullopt);
   ...
   */
  double estimate_error(const IonState &y4, const IonState &y5,
   ...
   * @return New timestep
   */
  double compute_new_step(double current_dt, double error, double dt_min,
                          double dt_max, double prev_error);
};
