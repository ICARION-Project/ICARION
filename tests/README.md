...
Current CPU-only build (`ctest -N`) exposes 76 tests. Each entry lists the CTest name followed by the source file.
...
- **Config:** `ConfigLoader` (`test_config_loader.cpp`), `ConfigOverride` (`test_config_override.cpp`), `WaveformTypes` (`test_waveform_types.cpp`), `WaveformLoader` (`test_waveform_loader.cpp`), `SpeciesLoaderSmoke` (`test_species_loader.cpp`), `SpeciesLoaderUnit` (`test_species_loader_unit.cpp`), `ReactionLoaderUnit` (`test_reaction_loader_unit.cpp`), `ReactionValidation` (`test_reaction_validation.cpp`), `DatabaseIntegration` (`test_database_integration.cpp`), `IonLoader` (`test_ion_loader.cpp`), `FieldArrayTermsLoader` (`test_field_array_terms_loader.cpp`), `FieldArrayE2E` (`test_field_array_e2e.cpp`)
- **I/O and utils:** `HDF5Writer` (`test_hdf5_writer.cpp`), `HDF5WriterV2` (`test_hdf5_writer_v2.cpp`), `test_hash` (`test_hash.cpp`)
...
- **Quarantined source:** `tests/integrator/quarantined/test_integration_batch_dt.cpp` is not part of CTest because it white-boxed the private `SimulationEngine::perform_integration()` method.
...
- **Integrators:** RK4, RK45 adaptive/fixed behavior, RK45 per-ion dt handling, Boris, adaptive space-charge stage refresh, boundary actions, RNG/compaction determinism
...
- **Config:** Loaders/validators for configs, species, reactions, ions, waveforms, field-array terms, field-array E2E, database integration
- **I/O:** HDF5 writer and HDF5 v2 regression coverage
