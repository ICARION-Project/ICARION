...
static constexpr double k_coulomb = 8.987551787e9;  // N·m²/C²
...
static IonState create_ion(Vec3 pos, double charge_C, double mass_u = 100.0) {
    ...
    ion.mass_kg = mass_u * 1.66053906660e-27;  // Convert u to kg
    return ion;
}
...
TEST_CASE("SpaceChargeForce: Applies to all ions", "[SpaceChargeForce][basic]") {
    SpaceChargeForce force{0.0};
    IonState ion = create_ion(Vec3{0, 0, 0}, e);
...
TEST_CASE("SpaceChargeForce: Single ion returns zero force", "[SpaceChargeForce][empty]") {
    ...
    ion_ensemble.push_back(create_ion(Vec3{0, 0, 0}, e));
...
TEST_CASE("SpaceChargeForce: Self-interaction excluded", "[SpaceChargeForce][self]") {
    ...
    ion_ensemble.push_back(create_ion(Vec3{1e-6, 0, 0}, e));
    ion_ensemble.push_back(create_ion(Vec3{2e-6, 0, 0}, e));
    ...
    const double expected_force_x = -k_coulomb * e * e / (1e-6 * 1e-6);
...
TEST_CASE("SpaceChargeForce: Two positive charges repel along X", "[SpaceChargeForce][coulomb]") {
    ...
    IonState ion0 = create_ion(Vec3{0, 0, 0}, e);
    IonState ion1 = create_ion(Vec3{1e-6, 0, 0}, e);
    
    ion_ensemble.push_back(ion0);
    ion_ensemble.push_back(ion1);
    
    // Force on ion0 from ion1 (should push left: negative x)
    Vec3 force0 = force.compute(ion0, 0.0, ctx);
    
    // Expected: F = k·e²/r²
    const double r = 1e-6;
    const double expected_magnitude = k_coulomb * e * e / (r * r);
...
TEST_CASE("SpaceChargeForce: Newton's third law", "[SpaceChargeForce][newton]") {
    ...
    IonState ion0 = create_ion(Vec3{0, 0, 0}, e);
    IonState ion1 = create_ion(Vec3{1e-6, 1e-6, 0}, 2.0 * e);  // Double charge
    
    ion_ensemble.push_back(ion0);
    ion_ensemble.push_back(ion1);
    
    Vec3 force_on_0 = force.compute(ion0, 0.0, ctx);
    Vec3 force_on_1 = force.compute(ion1, 0.0, ctx);
...
TEST_CASE("SpaceChargeForce: Three-dimensional repulsion", "[SpaceChargeForce][3d]") {
    ...
    IonState ion0 = create_ion(Vec3{0, 0, 0}, e);
    IonState ion1 = create_ion(Vec3{1e-6, 1e-6, 1e-6}, e);
...
TEST_CASE("SpaceChargeForce: Three ions linear", "[SpaceChargeForce][superposition]") {
    ...
    IonState ion0 = create_ion(Vec3{0, 0, 0}, e);
    IonState ion1 = create_ion(Vec3{1e-6, 0, 0}, e);
    IonState ion2 = create_ion(Vec3{2e-6, 0, 0}, e);
    ...
    const double F1 = -k_coulomb * e * e / (1e-6 * 1e-6);
    const double F2 = -k_coulomb * e * e / (2e-6 * 2e-6);
...
TEST_CASE("SpaceChargeForce: Softening prevents infinite force", "[SpaceChargeForce][softening]") {
    ...
    IonState ion0 = create_ion(Vec3{0, 0, 0}, e);
    IonState ion1 = create_ion(Vec3{1e-11, 0, 0}, e);  // 0.01 nm separation
    
    ion_ensemble.push_back(ion0);
    ion_ensemble.push_back(ion1);
    
    Vec3 force_no = force_no_soft.compute(ion0, 0.0, ctx);
    Vec3 force_with = force_with_soft.compute(ion0, 0.0, ctx);
...
TEST_CASE("SpaceChargeForce: Softening converges to Coulomb at large distances", "[SpaceChargeForce][softening]") {
    ...
    IonState ion0 = create_ion(Vec3{0, 0, 0}, e);
    IonState ion1 = create_ion(Vec3{100e-6, 0, 0}, e);
...
TEST_CASE("SpaceChargeForce: Overlapping ions return zero force", "[SpaceChargeForce][edge]") {
    ...
    IonState ion0 = create_ion(Vec3{1e-6, 1e-6, 1e-6}, e);
    IonState ion1 = create_ion(Vec3{1e-6, 1e-6, 1e-6}, e);
    
    ion_ensemble.push_back(ion0);
    ion_ensemble.push_back(ion1);
    
    Vec3 result = force.compute(ion0, 0.0, ctx);
...
TEST_CASE("SpaceChargeForce: Time independent", "[SpaceChargeForce][edge]") {
    ...
    Vec3 force_t0 = force.compute(ion0, 0.0, ctx);
    Vec3 force_t1 = force.compute(ion0, 1e-6, ctx);
    Vec3 force_t2 = force.compute(ion0, 1.0, ctx);
