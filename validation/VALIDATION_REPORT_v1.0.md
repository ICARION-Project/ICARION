...
- The normalized $A/\sqrt{N}$ check still shows 40–51 % variation because we only observe the first 20 µs of evolution; longer capture windows would further reduce that spread but are not required for v1.0.

Command reference: `python3 validation/scripts/physics/analyze_spacecharge.py validation/results/v1.0_test/physics/spacecharge`.

#### IMS Drift With/Without Space Charge

The IMS analyzer now runs end-to-end alongside the Coulomb fits and refreshes `validation/results/v1.0_test/physics/spacecharge/spacecharge_ims_comparison.png`. The updated statistics are:
...
All Coulomb-expansion datasets now contain the full 20 µs time series, the analyzer recreates the solver-parity plot without errors, and IMS broadening remains documented (+2 % at 15 k ions, ≈0 % at 50 k given the short drift distance). We still flag the large $A/\sqrt{N}$ variation for future tuning, but no open blockers remain for the v1.0 release.
